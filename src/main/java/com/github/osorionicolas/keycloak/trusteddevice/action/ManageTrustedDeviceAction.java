...
import com.github.osorionicolas.keycloak.trusteddevice.TrustedDeviceCookie;
import com.github.osorionicolas.keycloak.trusteddevice.TrustedDeviceName;
import com.github.osorionicolas.keycloak.trusteddevice.TrustedDeviceToken;
import com.github.osorionicolas.keycloak.trusteddevice.account.AccountEmail;
import com.github.osorionicolas.keycloak.trusteddevice.credentials.TrustedDeviceCredentialModel;
import com.github.osorionicolas.keycloak.trusteddevice.credentials.TrustedDeviceCredentialProvider;
import com.github.osorionicolas.keycloak.trusteddevice.support.RealmUtils;
import com.github.osorionicolas.keycloak.trusteddevice.support.RequiredActionUtils;
import com.github.osorionicolas.keycloak.trusteddevice.account.MfaChange;
...
public class ManageTrustedDeviceAction implements RequiredActionProvider {
        ...
        if (formParams.containsKey("dont-trust-device")) {
            ...
            if (trustedDeviceModel != null) {
                ...
                if (deleted) {
                    ...
                    this.onTrustedDeviceChange(session, realm, user, new TrustedDeviceInfo(trustedDeviceModel.getUserLabel()), MfaChange.REMOVE);
                }
            }
        }
    ...
    private void afterTrustedDeviceRegistration(RequiredActionContext context, TrustedDeviceInfo trustedDeviceInfo) {
        ...
        this.onTrustedDeviceChange(context.getSession(), context.getRealm(), context.getUser(), trustedDeviceInfo, MfaChange.ADD);
    }
    ...
    private void onTrustedDeviceChange(KeycloakSession session, RealmModel realm, UserModel user, TrustedDeviceInfo trustedDeviceInfo, MfaChange change) {
        try {
